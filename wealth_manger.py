...
class Calculator:
    ...
    def _calculate(self):
        ...
        while self._passive_income_desired_yearly >= (apt_number_owned * self._price_of_renting_one_apt):
            ...
            if whole >= 2:
                apt_number_owned += whole  # round this to an int because we want full numbers.
                income_reached = (self._yearly_savings + income_reached) - (whole * self._price_of_one_apt)
                income_reached += (apt_number_owned * self._price_of_renting_one_apt)  # get rent passive income
            elif whole >= 1:
                apt_number_owned += whole  # round this to an int because we want full numbers.
